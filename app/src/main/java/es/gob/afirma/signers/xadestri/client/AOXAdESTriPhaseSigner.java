...
public class AOXAdESTriPhaseSigner implements AOSigner {
	...
	protected static byte[] triPhaseOperation(final String format,
		...
		try {
			...
			preSignResult = urlManager.readUrl(postUrl, -1, "application/xml", "text/xml", UrlHttpMethod.POST);
		...
		try {
			...
			triSignFinalResult = urlManager.readUrl(urlBuffer.toString(), -1, "application/xml", "text/xml", UrlHttpMethod.POST);
